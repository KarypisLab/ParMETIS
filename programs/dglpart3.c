...
graph_t *DGLPart_ReadGraph(char *fstem, MPI_Comm comm)
{
  ...
  /* ======================================================= */
  {
    ...
    for (chunk=0;;chunk++) {
      ...
      for(pe=0; pe<npes; pe++)
	for(i=0; i<coo_buffers_cpos[pe]; i++)
          lcoo[lnedges++] = coo_buffers[pe][i];
    }
    nchunks = chunk;
    ...
    gk_free((void **)&coo_buffers, LTERM);
  ...
  /* ======================================================= */
  {
    ...
    for (chunk=0;;chunk++) {
      ...
      /* consolidate vertices here */
      for(pe=0; pe<npes; pe++){
        for(i=0; i<con_buffers_cpos[pe]; i+=ncon, lvtxs++){
	  vtype[lvtxs] = con_buffers[pe][ncon*i+0];
	  for(j=1; j<ncon; j++){
	    vwgt[(ncon-1)*lvtxs+j-1] = con_buffers[pe][ncon*i+j];
	  }
	}
      }
    }
    ...
    gk_free((void **)&con_buffers, LTERM);
